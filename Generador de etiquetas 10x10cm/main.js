...
function crearEtiquetas(codigos) {
    ...
    codigos.forEach(element => {
        ...
        div.innerHTML = `<img class="logo" src="/TuLogoAqui.png">
        <br>
        ...
        <h3 class="descriptionFooter" style="color: #000; ">www.github.com/MXavierK</h3>
        </div>`;
...
function descargarImagenes() {
    ...
    const elementosPorGrupo = 150; // O 100, según prefieras
    const totalGrupos = Math.ceil(elementos.length / elementosPorGrupo);
    const maxTiempo = 30000; // Límite de tiempo en milisegundos (60 segundos)
    let grupoActual = 0;
    let descargarSiguienteGrupo = () => {
        ...
        Promise.all(promises).then((canvases) => {
            ...
            // Descargar el siguiente grupo de imágenes después de un cierto tiempo
            grupoActual++;
    ...
    setTimeout(() => {
        ...
        // Mostrar un mensaje al usuario informándole que el proceso ha tardado demasiado y que debe intentarlo de nuevo más tarde o descargar los archivos generados hasta ese momento.
    }, maxTiempo);
}
